...
def run_stats_benchmark():
    ...
    if not os.path.exists(repo_path):
        # Fallback to src directory if needed, but normally integration tests clone it to tests/data/requests
        ...
        try:
            ...
            for chunk in chunks:
                if chunk.metadata.node_type == "function":
                    total_functions += 1
                    # If the function content exceeds CHUNK_SIZE, 
                    # a fixed-size splitter would inevitably break it.
    ...
    print(f"Broken by Fixed-size (800):   {broken_functions_fixed_800}")
    print(f"Broken by PyAST-RAG:          0")
    
    broken_ratio = (broken_functions_fixed_800 / total_functions) * 100
    print(f"Broken Rate (Fixed-size):     {broken_ratio:.2f}%")
    print("="*40)
